...
void ofApp::draw()
{
    ...
    for (auto square: squares)
    {
        float drawLocationX = ofMap(square.featureProjection[0], 0, 1, 0, ofGetWidth());
        float drawLocationY = ofMap(square.featureProjection[1], 0, 1, 0, ofGetHeight());

        square.tex.draw(drawLocationX, drawLocationY);
    }
}
