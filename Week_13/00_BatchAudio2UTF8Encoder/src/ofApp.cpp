...
void ofApp::setup()
{
    ...
    fileFilter = std::make_unique<ofxIO::RegexPathFilter>(settings.value("filename_filter", ".*wav"));
    
    std::size_t numWorkers = settings.value("num_workers", 8);

    for (std::size_t i = 0; i < numWorkers; ++i)
    {
        ofJson encoderSettings = settings["audio_encoder"];

        auto encoder = std::make_unique<AudioToUTF8Encoder>(encoderSettings);
        encoder->start();
        encoders.push_back(std::move(encoder));
    }

    // It will start with these images.
    std::string images = settings["audio_encoder"]["initial_directory_to_process"];
    process(images);
...
void ofApp::draw()
{
    ...
    ofDrawBitmapString("Drop audio files / folders\nto encode.", x, y);
